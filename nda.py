...
if internettest == 1:
	...
	if 'y' in autoupdatev.lower():
		...
		try:
			...
			print 'Although internet test passed, found issue with downloading the version file. Is the system being proxied? Please whitelist github.com'
			skipupdate = 1
		if skipupdate == 0:
...
if __name__ == "__main__":
	...
	startofthread = threading.activeCount()
	for sshdevice in sshdevices:	
...
if fullinventoryreportv == 1 and fullinventorylist != []:
	...
	print 'No data retrieved from the network. Skipping Full Inventory Report.'
if interfacereportv == 1 and l2interfacelist != []:
	print 'Exporting Interface Report'
	interfacereport(l2interfacelist,l3interfacelist,poeinterfacelist,exportlocation)
else:
	print 'No data retrieved from the network. Skipping Interface Report.'
if poereportv == 1 and poeinterfacelist != []:
	print 'Exporting POE Report'
	poereport(poeinterfacelist,exportlocation)
else:
	print 'No data retrieved from the network. Skipping POE Report.'
if healthcheckv == 1 and healthchecklist != []:
	print 'Exporting Health Report'
	healthcheckreport(healthchecklist,exportlocation)
else:
	print 'No data retrieved from the network. Skipping Health Report.'
if arpmacreportv == 1 and ipmactablelist != []:
	print 'Exporting ARP/MAC Report'
	arpmacreport(iparptablelist,ipmactablelist,mactablelist,exportlocation)
else:
	print 'No data retrieved from the network. Skipping ARP/MAC Report.'
