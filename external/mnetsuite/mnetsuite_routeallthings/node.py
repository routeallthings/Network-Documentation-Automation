...
class mnet_node:
	...
	def query_node(self):
		...
		hpproductreg = re.compile('^[A-Z][0-9]{3}[A-Za-z0-9]?$')
		
		modelvendor = ''
		if any(word in modeltype for word in ciscoioslist):
			modelvendor = 'cisco_ios'
		if any(word in modeltype for word in ciscoxelist):
			modelvendor = 'cisco_ios'
		if any(word in modeltype for word in cisconxoslist):
			modelvendor = 'cisco_nxos'
		if re.match(hpproductreg, modeltype):
			modelvendor = 'hp_procurve'
		if modelvendor == '':
			modelvendor = 'cisco_ios'
		
		# Switch out OIDs for correct vendor
		...
		if (self.opts.get_router == True):
			if (self.router == None):
				self.router = 1 if (snmpobj.get_val(OID_IP_ROUTING) == '1' or snmpobj.get_val(OID_IP_ROUTING) == '2') else 0
